...
class NormalSanadSarfReport extends Controller
{
    use LogActivityTrait;
    //
    public function index(Request $request)
    {
        $this->add_log_activity(null,auth()->user(),'ترحيل سندات الصرف النقدية');


        //return all Suppliers
        if ($request->ajax()) {
            
            $datas = Bond::where(['type'=>'normal_snd_alsirf', 'is_confirmed'=> 'new', 'payment_type'=>'cash'])->with('debit_rl','creditor_rl')->orderby('id','DESC')->get();
            ...
            return DataTables::of($datas)
                ...
                ->addColumn('actions', function ($data) {
                        ...
                        $html = "
                            <a  class='btn mb-2 btn-success ' href='".route('convertSandSarf', $data->id)."' id='" . $data->id . "'> تحويل</a>";
        ...
        return view('admin.modules.Finances.Reports.sanads_grid')->with(['route_url'=>route('NormalSandSarfReport'), 'page_title'=>'ترحيل سندات الصرف', 'show_actions'=>true]);
    }
    
    public function convert($id)
    {
        $this->add_log_activity(null,auth()->user(),'عمليةترحيل سندات الصرف النقدية');
    ...
    {
        $this->add_log_activity(null,auth()->user(),'  سندات الصرف النقديةالمؤكدة');


        //return all Suppliers
        if ($request->ajax()) {
            
            $datas = Bond::where(['type'=>'normal_snd_alsirf', 'is_confirmed'=> 'confirmed', 'payment_type'=>'cash'])->with('debit_rl','creditor_rl')->orderby('id','DESC')->get();
        ...
        return view('admin.modules.Finances.Reports.sanads_grid')->with(['route_url'=>route('sanadSarfConfirmedList'), 'page_title'=>'سندات الصرف المؤكدة', 'show_actions'=>false]);
    }
}
