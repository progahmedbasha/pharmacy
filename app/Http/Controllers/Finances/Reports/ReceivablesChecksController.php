...
class ReceivablesChecksController extends Controller
{
    //شيكات سندات قبض
    ...
    {
        $this->add_log_activity(null,auth()->user(),'ترحيل سندات القبض شيكات');


        //return all Suppliers
        if ($request->ajax()) {
            
            $datas = Bond::where(['type'=>'normal_snd_alqabd', 
                                  'is_confirmed'=> 'new', 
            ...
            return DataTables::of($datas)
                ...
                ->addColumn('actions', function ($data) {
                        ...
                        $html = "
                            <a  class='btn mb-2 btn-success ' href='".route('ReceivablesChecksConvert', $data->id)."' id='" . $data->id . "'> ترحيل</a>";
        ...
        return view('admin.modules.Finances.Reports.checks_grid')->with(['route_url'=>route('ReceivablesChecksReport'), 'page_title'=>'ترحيل سندات القبض', 'show_actions'=>true]);
    }
    
    public function convert($id)
    {
        $this->add_log_activity(null,auth()->user(),'عمليةترحيل سندات القبض شيكات');
        
        //check bond exist
        $bond_data = Bond::findOrFail($id);
        
        //check catch paper account exist
        $account_data = Account::where('type', 'catch_papers')->first();
    ...
    public function ReceivablesChecksTodayConfirmedList(Request $request)
    {
                ...
                return DataTables::of($table_data)
                   ...
                   ->editColumn('creditor_rl', function ($data) {
                    ...
                    ->addColumn('actions', function ($data) {
                        ...
                        {
                            $html = "
                                <a  class='btn mb-2 btn-success ' href='".route('confirmCheck', $data->id)."' id='" . $data->id . "'> تأكيد</a>";
                        }
                        else
                        {
                            $html = "
        ...
        return view('admin.modules.Finances.Reports.confirmed_checks')->with(['route_url'=>route('ReceivablesChecksTodayConfirmedList'), 'page_title'=>'تحصيل أوراق القبض اليوم', 'show_actions'=>true]);
    }
    
    public function ReceivablesChecksAllConfirmedList(Request $request)
    {
        ...
        return view('admin.modules.Finances.Reports.confirmed_checks')->with(['route_url'=>route('ReceivablesChecksAllConfirmedList'), 'page_title'=>'تحصيل كل أوراق القبض ', 'show_actions'=>true]);
    }
