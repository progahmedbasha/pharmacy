...
<br><br>
 ...
 <form action="{{route('purchase_return_bill')}}" method="post">
  @csrf
<div class="row">
...
<br><br>
  ...
  <input type="hidden" name="supp_id" value="{{$bill->supplier_id}}"/>
<div class="table-responsive">
    <table class="table table-bordered table-striped main" id="dynamicTable">  
      ...
      @foreach($bill->bill_products as $index=>$item)
        @if($item->product_date->quantity > 0)
          <tr>  
            <td><input type="hidden" name="multi_product[]" value="{{$item->id}}"/>
              {{$item->product_date->product->item->name}}</td> 
            <td>{{$item->product_date->production_date}}</td> 
            <td>{{$item->product_date->expire_date}}</td>
            <td><input type="number" step="0.001"  class="form-control" value="{{$item->product_date->cost}}" id="price{{$index}}" disabled/></td> 
            <td><input type="number" class="form-control" name="multi_quantity[]" value="{{$item->product_date->quantity}}" id="quantity{{$index}}" onChange="check_quantity({{$index}})" onkeyup="this.onchange();" onpaste="this.onchange();" oninput="this.onchange();"/></td>  
...
<br><br>
@if($bill->supplier->type == 0 || ($bill->supplier->type == 1 && $bill->supplier->tree->balance >= 0))
<div class="row">
...
<script type="text/javascript">
  ...
  var pro = <?php echo  $bill->bill_products; ?>;
